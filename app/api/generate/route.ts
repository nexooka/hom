...
function buildGeminiPrompt(userPrompt: string): string {
  return [
    ...
    `=== GENERATE: ${userPrompt} ===`,
    '',
    '=== FOLLOW THE PROMPT EXACTLY — NON-NEGOTIABLE ===',
    `The user said: "${userPrompt}"`,
    '• Every single detail the user described MUST appear visibly in the image.',
    '• Do not substitute, simplify, or ignore any part of the prompt.',
    '• If the user said "holding a coffee cup" — there is a coffee cup. If they said "wearing a hat" — there is a hat. No exceptions.',
    '• If the prompt specifies an emotion or action, that emotion/action must be unmistakably obvious.',
    '• Do not add things the user did not ask for. Do not remove things the user asked for.',
    '• The image must be a 1-to-1 literal visual translation of the prompt.',
    '',
    '=== FINAL CHECK ===',
